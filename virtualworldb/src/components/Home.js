...
function Home({ username }) {
    ...
    useEffect(() => {
        ...
        if (storedUsername) {
            fetch(`http://127.0.0.1:8000/user/${storedUsername}`)
                .then(response => {
                    if (!response.ok) {
    ...
    const profilePhotoUrl = userData.profilePhoto ? `http://127.0.0.1:8000/static/${userData.profilePhoto.split("media/")[1]}` : Pic;
